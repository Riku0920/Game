...
public class GunScript : MonoBehaviourPunCallbacks {
	...
	void Awake(){
		...
		PS = player.GetComponentInParent<PlayerStatus>();
	}
	...
	*/
	...
	void Shooting(){
		...
		if (!meeleAttack && OS.GameTime && SS.authenticity) {
	...
	*/
	...
	private void ShootMethod(){
		...
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
                ...
                {
					SetAttack();
					GameObject Gun = PhotonNetwork.Instantiate("bullet", bulletSpawnPlace.transform.position, bulletSpawnPlace.transform.rotation);
					Gun.GetComponent<BulletScript>().SetAtk(GunAtk);
				}
	...
	{
		GunAtk = this.gameObject.GetComponent<GunStatus>().GunStatusAtk;
	}
	...
	*/
	...
	private SettingScript SS;
    private void Start()
    {
		setting = GameObject.FindGameObjectWithTag("Setting");
		SS = setting.GetComponent<SettingScript>();
		Mousesensitivity MS = setting.GetComponent<Mousesensitivity>();
		float SliderV = MS.SliderValue;

		mouseSensitvity_notAiming = SliderV;
		mouseSensitvity_aiming = SliderV;
		mouseSensitvity_running = SliderV;
	}
    void OnGUI(){
		if(!HUD_bullets){
			try{
				HUD_bullets = GameObject.FindGameObjectWithTag("hub").GetComponent<Text>();
			}
		...
		if(mls && HUD_bullets)
			HUD_bullets.text = " Bullets: " + bulletsInTheGun.ToString();
